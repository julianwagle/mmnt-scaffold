...
const ProductFooter = ({ meta, handler }) => {
  ...
  return (
    <CardFooter tag={Flex} justify="center" align="center" className="bg-light border-top">
      <Button color="mysite-default" size="sm" className="ml-1 ml-sm-2" onClick={prevPage} disabled={!prevPageNo}>
        <FontAwesomeIcon icon={`chevron-${isRTL ? 'right' : 'left'}`} />
      </Button>
      {getPaginationArray(total, itemsPerPage).map(page => (
        <Button
          color={pageNo === page ? 'mysite-primary' : 'mysite-default'}
          size="sm"
          className="ml-2"
      ...
      <Button color="mysite-default" size="sm" className="ml-1 ml-sm-2" onClick={nextPage} disabled={!nextPageNo}>
